...
export class CodeRunner {
  ...
  public async runCode(code: string) {
    ...
    try {
      ...
      this.setState(RunState.Ready, interrupted ? `Execution was interrupted at ${(new Date().getTime() - this.runStartTime) / 1000}s` : `Executed in ${(new Date().getTime() - this.runStartTime) / 1000}s`);
      if (terminated) {
        await this.start();
